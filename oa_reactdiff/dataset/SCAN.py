...
from oa_reactdiff.dataset.base_dataset import BaseDataset

#STRECKER_ATOM_MAPPING = {
#    1: 0,
#    6: 1,
#    7: 2,
#    8: 3,
#    9: 4,
#} # Strecker:['H', 'C', 'N', 'O', 'Na', 'Cl']

# SCAN (7 reaction maps) ATOM_MAPPING:
...
n_element_SCAN = len(list(ATOM_MAPPING_SCAN.keys()))
...
class ProcessedSCAN(BaseDataset):
    def __init__(
        self,
        npz_path,
        center=True,
        ...
        **kwargs,
        ...
        if confidence_model:
            use_by_ind = False
        if remove_h:
            print("remove_h is ignored because it is not reasonable for TS.")
        if single_frag_only:
